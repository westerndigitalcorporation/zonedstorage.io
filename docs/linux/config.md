...
title="Block layer zoned block device support option with make menuconfig"/>
...
Starting with kernel 6.10, write ordering control is implemented by the block
...
After the kernel user header files have been installed, we recommend that
...
user API header files. The RPM package `kernel-headers-<version>.<arch>.rpm`
